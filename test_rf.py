...
def evaluate_rf(model, X, y, set_name):
    ...
    save_metrics(metrics, f'rf_metrics_{set_name}.txt')
    
    print(f"\nRF Performance on {set_name} set:")
    print("="*50)
...
def plot_curves(y_true, y_score, set_name):
    ...
    plt.legend()
    plt.savefig(os.path.join(RESULTS_DIR, f'RF_roc_{set_name}.png'))
    plt.close()
    ...
    plt.legend()
    plt.savefig(os.path.join(RESULTS_DIR, f'RF_pr_{set_name}.png'))
    plt.close()
    ...
    plt.legend()
    plt.savefig(os.path.join(RESULTS_DIR, f'RF_accuracy_vs_threshold_{set_name}.png'))
    plt.close()
    ...
    plt.legend()
    plt.savefig(os.path.join(RESULTS_DIR, f'RF_f1_vs_threshold_{set_name}.png'))
    plt.close()

def test_rf():
    """Main function to evaluate RF on development and test sets"""
    try:
        ...
        try:
            print("\nEvaluating RF on development set...")
            X_dev, y_dev = data_loader.load_features('development')
            evaluate_rf(rf, X_dev, y_dev, "development")
        ...
        evaluate_rf(rf, X_test, y_test, "test")
                
    except Exception as e:
        print(f"\nCritical error during RF testing: {str(e)}")
        raise
